...
def train_step(batch,model,mp,criterion,optimizer):
    ...
    batch_images = detatch(batch_images)

    batch_labels = detatch(batch_labels)

    batch_predictions = detatch(batch_predictions)
...
class R2_CHs(object):
    ...
    * R2_CHs: class to calculate the regression metric R^2 between the predicted CHs
    ...
    def __init__(self,batch_predictions,batch_labels):
        ...
        self.batch_labels = np.array(batch_labels.detach().cpu())
